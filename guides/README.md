...
| [Export a `cookies.txt` (desktop & Android)](cookies-txt-export-desktop-android.md) | Give `yt-dlp` / `gallery-dl` a logged-in session: the Netscape `cookies.txt` format, exporting from desktop or from an Android phone/tablet (via the Ultimatum browser), where the file goes, rotation, and staying un-banned. |
